const birthDate = { year: 2005, month: 3, day: 23 };
const today = new Date();
let currentAge = today.getFullYear() - birthDate.year;
...
const tetrisDialog = document.querySelector("#block-drop-preview");
const openTetrisButton = document.querySelector("[data-open-tetris]");
const closeTetrisButton = document.querySelector("[data-close-tetris]");
const tetrisFrame = tetrisDialog.querySelector("iframe");

const openTetrisPreview = () => {
  if (typeof tetrisDialog.showModal === "function") tetrisDialog.showModal();
  else tetrisDialog.setAttribute("open", "");
  document.body.classList.add("phone-preview-open");
  closeTetrisButton.focus();
};

const closeTetrisPreview = () => {
  if (typeof tetrisDialog.close === "function") tetrisDialog.close();
  else tetrisDialog.removeAttribute("open");
  document.body.classList.remove("phone-preview-open");
  tetrisFrame.contentWindow?.postMessage({ type: "block-drop-pause" }, "*");
  openTetrisButton.focus();
};

openTetrisButton.addEventListener("click", openTetrisPreview);
closeTetrisButton.addEventListener("click", closeTetrisPreview);
tetrisDialog.addEventListener("click", (event) => {
  if (event.target === tetrisDialog) closeTetrisPreview();
});
tetrisDialog.addEventListener("close", () => document.body.classList.remove("phone-preview-open"));
